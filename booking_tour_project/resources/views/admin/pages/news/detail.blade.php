            ...
            <div class="col-12">
                <div class="card card-secondary">
                    <div class="card-header">
                        <h3 class="card-title">{{__('admin_news.detail_title')}}</h3>
                    </div>
                    <!-- /.card-header -->
                    <!-- form start -->
                        ...
                        <div class="card-body">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control {{$errors->has('title') ? 'is-invalid' : ''}}"
                                       name="title"
                                       placeholder="{{__('admin_news.title_note')}}" value="{{ $new->title }}"/>
                                <div style="color: red">
                                    @error('title')
                                    {{$message}}
                                    @enderror
                            ...
                            <div class="form-group" id="add-item">
                                ...
                                <textarea id="my-editor" name="content"
                                          class="form-control {{$errors->has('content') ? 'is-invalid' : ''}}">{{$new->content}}</textarea>
                                <div style="color: red">
                                    @error('content  ')
                                    {{$message}}
                                    @enderror
...
@push('script')
    ...
    <script type="text/javascript" src="{{asset('dist/js/admin_news.js')}}"></script>
@endpush
